...
pub fn route(tera: &Tera, template_name: &str, output: &str, context: Context) {
  // Log the update
  Lib::print("green", "Render", "static", &output);
  // Render the given template
  render(tera, template_name, output, context);
}
...
pub fn route_group(tera: &Tera, template_name: &str, templates: Vec<Template>) {
  for template in templates {
    // Log the update
    Lib::print("green", "Render", "static", &template.output);
    // Render the current template
    render(tera, template_name, template.output, template.context);
  }
}